#!/usr/bin/env python3
"""Korrigiert M75_membres_2026_2027_Codes_alt_neu_130726.xlsx:
1) Tables new: Labels konsequent Femmes/FE (net Dames), Mixte no Geschlecht
2) Neue Nummern (100er) für Verwaltungs-/Zahlungscodes
3) CAT_JOUEUR -> intern 'Tables' umbiegen
4) K/N mit VLOOKUP-Formeln füllen (alter Code -> neuer Code)
5) Datentypen: Codes als ZAHL (nicht Text)
"""
import openpyxl
...
# Labels: Femmes/FE, Mixte nach Geschlecht
LABEL = {
    '2': 'Seniors HO', '3': 'U21 HO', '4': 'U17 H', '5': 'U15 H', '6': 'U13 H',
    '7': 'U11 H', '8': 'U9 H', '9': 'Vétérans HO',
    '10': 'Arbitre HO (21) / FE (41)', '21': 'U7 H', '20': 'U4 H',
    '1': 'Officiel HO', '11': 'Officiel FE',
    '12': 'Seniors FE', '14': 'U17 FE', '15': 'U13 F', '16': 'U15 F',
    '17': 'U11 F', '18': 'U9 F',
...
for r in range(2, tn.max_row + 1):
    ...
    # Label (Spalte E) - Femmes/FE
    if code == '19':
        tn.cell(r, 5).value = 'Vétérans FE' if ('éran' in desc.lower() or 'eran' in desc.lower()) else 'U7 F'
    elif code in LABEL:
        tn.cell(r, 5).value = LABEL[code]
    # Neue Nummer (Spalte C) + Bereich (Spalte D)
    if code in NEWCODE:
        tn.cell(r, 3).value = NEWCODE[code]
        tn.cell(r, 4).value = ADMIN_BEREICH

# 995 / 998 anhängen falls fehlend
...
# ── 2) Datentypen: Spalten A und C als ZAHL ────────────────
for r in range(2, tn.max_row + 1):
    a = tn.cell(r, 1).value
    if isinstance(a, str) and a.strip().isdigit():
        tn.cell(r, 1).value = int(a.strip())
    c = tn.cell(r, 3).value
    if isinstance(c, str) and c.strip().isdigit():
        tn.cell(r, 3).value = int(c.strip())

# ── 3) CAT_JOUEUR auf intern 'Tables' umbiegen ────────────────
dn = wb.defined_names['CAT_JOUEUR']
dn.value = "'Tables'!$A$1:$B$77"

# ── 4) Membres: K/N Formeln (alter Code -> neuer Code) ─────
