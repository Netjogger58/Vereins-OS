// Komplette Zuordnung ALLER alten Codes (Quelle: Blatt "TABLES" der GC-Memberslëscht)
// -> neuer Code + Erklärung (docs/kategorien-neuordnung.md). Blatt "Tabelle1".
// Jede einzelne Nummer aus der Original-Codeliste bekommt eine Zeile.
...
// Zuordnung alter Code -> { neu, bereich, zuordnung }.
// Für den doppelt vergebenen Code 19 (U7 Dames / Vétérans Dames) wird der Text ausgewertet.
function mapCode(code, desc) {
  ...
  // Code 19 doppelt vergeben (U7 Dames / Vétérans Dames)
  if (code === '19') {
...
const rows = XLSX.utils.sheet_to_json(wb.Sheets['TABLES'], { header: 1, defval: '', raw: false });

const out = [['Alter Code', 'Original-Beschreibung (FR)', 'Neuer Code', 'Zuordnung (Bereich)', 'Erklärung / Neu']];
let n = 0;
for (let i = 0; i < rows.length; i++) {
  ...
  out.push([code, desc, mp.neu, mp.bereich, mp.z]);
  n++;
}

const ws = XLSX.utils.aoa_to_sheet(out);
ws['!cols'] = [{ wch: 10 }, { wch: 42 }, { wch: 11 }, { wch: 22 }, { wch: 52 }];
ws['!autofilter'] = { ref: XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: out.length - 1, c: 4 } }) };

const owb = XLSX.utils.book_new();
XLSX.utils.book_append_sheet(owb, ws, 'Tabelle1');
XLSX.writeFile(owb, OUT_PATH);

console.log(`Komplett: ${n} Codes (alle aus TABLES) -> gespeichert:`, OUT_PATH);
