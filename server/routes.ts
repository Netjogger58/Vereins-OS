...
export async function registerRoutes(_httpServer: Server, app: Express): Promise<Server> {
  ...
  app.post("/api/auth/identify-member", async (req: AuthedRequest, res: Response) => {
  ...
  app.post("/api/auth/register-otp", async (req: AuthedRequest, res: Response) => {
  ...
  app.post("/api/auth/register-complete", async (req: AuthedRequest, res: Response) => {
  ...
  app.post("/api/auth/pin-login", async (req: AuthedRequest, res: Response) => {
  ...
  app.post("/api/auth/pin-reset-request", async (req: AuthedRequest, res: Response) => {
  ...
  app.post("/api/auth/pin-reset-complete", async (req: AuthedRequest, res: Response) => {
    const { memberId, otpCode, pin, countryCode = "+352", method = "sms" } = req.body || {};
    if (!memberId || !otpCode || !pin) {
  ...
  app.get("/api/events/:eventId/groups", requireAuth(), async (req, res) => {
  ...
  app.post("/api/events/:eventId/groups", requireAuth(["präsident", "admin", "trainer"]), async (req, res) => {
    const eventId = Number(req.params.eventId);
  ...
  app.delete("/api/event-groups/:groupId", requireAuth(["präsident", "admin", "trainer"]), async (req, res) => {
  ...
  app.post("/api/event-groups/:groupId/members", requireAuth(["präsident", "admin", "trainer"]), async (req, res) => {
    const groupId = Number(req.params.groupId);
    const memberId = Number(req.body.memberId);
    if (!memberId) return res.status(400).json({ message: "memberId erforderlich" });
  ...
  app.delete("/api/event-groups/:groupId/members/:memberId", requireAuth(["präsident", "admin", "trainer"]), async (req, res) => {
    const groupId = Number(req.params.groupId);
  ...
  app.post("/api/matches/:matchId/lineup", requireAuth(["präsident", "admin", "trainer"]), async (req, res) => {
    const matchId = Number(req.params.matchId);
